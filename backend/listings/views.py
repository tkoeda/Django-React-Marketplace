...
class HomePageListingsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        first_image = ListingImage.objects.filter(
            listing=OuterRef('pk')
        ).order_by('order')  

        return FurnitureListing.objects.filter(
            status='published'
        ).select_related('seller').annotate(
            first_image_id=Subquery(first_image.values('id')[:1]),
            first_image_url=Subquery(first_image.values('image')[:1])
        ).order_by('-created_at')
...
class MyPageListingsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = FurnitureListing.objects.filter(seller=self.request.user, status=status)
...
class ListingDetailView(generics.RetrieveAPIView):
    ...
    def get_object(self):
        ...
        try:
            listing =  FurnitureListing.objects.select_related('seller').prefetch_related('images').get(
                id=listing_id,
                status='published'
            )
            ...
            raise APIException(f"Published FurnitureListing with id {listing_id} not found.")
